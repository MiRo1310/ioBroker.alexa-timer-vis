  ...
  async onReady() {
    ...
    this.on("stateChange", async (id, state) => {
      ...
      if (
        ...
        let doNothing = false;
        if (
          timerObject.timerActiv.data.notNotedSentence.find(
        ...
        if (
          (value.indexOf("timer") >= 0 ||
            value.indexOf("stelle") >= 0 ||
            value.indexOf("stell") >= 0) &&
          value.indexOf("wecker") == -1
        ) {
          this.log.debug(
            "---------------------Start------------------------------"
          );
          ...
          let changeValue;
          let extendTimer = false;
          if (
            typeof toDo == "string" &&
            ["shortenTimer", "extendTimer"].includes(toDo)
          ) {
            extendTimer = true;
            //TODO - input abschneiden, damit er richtig verarbeitet werden kann, am besten in funktion auslagern
            let result = cutValue(value);
            value = result.val;
            changeValue = result.change;
            this.log.debug("todo " + JSON.stringify(toDo));
          }
          // Timer länge ermitteln aus Eingabe String und vergleichen
          const returnArrayDecomposeInputValue = decomposeInputValue(value);
          /**@type {string} Name of Timer */
          let name = returnArrayDecomposeInputValue[0];
          /**@type {number} Secondos of the Timer to abort*/
          let filteredInputSeconds = returnArrayDecomposeInputValue[1];
          let deleteTimerIndex = returnArrayDecomposeInputValue[2];
          const inputString = returnArrayDecomposeInputValue[3];
          ...
          compareCreationTimeAndSerial(debounceTime).then((val) => {
            if (
              (!val[0] &&
                ...
                value != "" &&
                !doNothing &&
                valueTimeOld != returnArrayDecomposeInputValue[1]) ||
              varInputContainsDelete ||
              extendTimer
            ) {
              // Wert als Alten Wert speichern um beim Triggern zu vergleichen
              if (typeof state.val == "string") {
                valueOld = state.val;
              }
              valueTimeOld = filteredInputSeconds;
              ...
              if (toDo == "deleteTimer") {
                ...
                if (value.indexOf(",") != -1) {
                  ...
                  // Timer anhalten
                  this.log.debug(
                    "Output (Name, Sec, DeleteTimerIndex): " +
                      JSON.stringify(returnArrayDecomposeInputValue)
                  );
                }
                findTimer(
                  filteredInputSeconds,
                  name,
                  deleteTimerIndex,
                  value,
                  questionAlexa
                  ...
                  if (timers) {
                    this.log.debug(
                      "Timers to delete: " + JSON.stringify(timers)
                    );
              ...
              // Timer soll erstellt werden
                ...
                if (filteredInputSeconds && filteredInputSeconds != 0) {
                  ...
                  if (!nameExist) {
                    ...
                    // Timer starten
                    startTimer(filteredInputSeconds, name, inputString);
                ...
                deleteTimerIndex = 1;
                //TODO Extend Timter
                this.log.debug("Input: " + JSON.stringify(value));
                let timerSeconds = decomposeInputValue(changeValue)[1];
                findTimer(
                  filteredInputSeconds,
                  name,
                  deleteTimerIndex,
                  ...
                  if (timer) {
                    this.log.debug("Timer to update: " + JSON.stringify(timer));
                    this.log.debug(
                      "ChangeValue: " + JSON.stringify(timerSeconds)
                    );
                    if (toDo == "shortenTimer")
                      timerSeconds = timerSeconds * -1;
                    // Endtimer darf nicht kleiner sein als der Starttimer
                    let changeTimer = timerObject.timer[timer];
                    let newEndTime = changeTimer.endTime + timerSeconds * 1000;
                    if (newEndTime > changeTimer.startTime) {
                      changeTimer.endTime += timerSeconds * 1000;
                      changeTimer.end_Time = time(changeTimer.endTime);

                      changeTimer.onlySec += timerSeconds;
                      this.log.debug(
                        "test " + JSON.stringify(changeTimer.onlySec)
                      );
                    }

                    //TODO - sekunden und prozent anpassen
                  }
                });
              }
            }
     ...
     */
    const cutValue = (val) => {
      this.log.debug("Value to Split: " + JSON.stringify(val));

      let valueSplit = {
        val: value.split("um")[0],
        change: value.split("um")[1],
      };
      this.log.debug("Value Splitted " + JSON.stringify(valueSplit));
      return valueSplit;
    };
    //ANCHOR FUNKTION getToDo
    /**
     *
     * @param {string} value Spracheingabeert
    ...
    const getToDo = (value) => {
      ...
      for (const array in arraysTodo) {
        if (abortLoop) break;
        // this.log.debug("array " + JSON.stringify(arraysTodo[array]));
    ...
    const compareCreationTimeAndSerial = async (debounceTime) => {
      const creationTime = await this.getForeignStateAsync(
        "alexa2.0.History.creationTime"
      );
      const serial = await this.getForeignStateAsync(
    ...
    //ANCHOR -  DecomposeInputValue
    /**
     *
     * @param {string} value Sprach-Eingabewert
     * @param {boolean} log Soll das Log ausgegeben werden?
     * @returns [] Name, Timer Zeit, Index zum Löschen der Timer(1 = ein Timer, 2 alle Timer), Input ausgeschrieben
     */
    const decomposeInputValue = (value, log = false) => {
      ...
      const returnArray = processInput(inputDecomposed, log);
      // Name
      let name = "";
    ...
    const oneOfMultiTimerDelete = (input, timeSec, name, inputDevice) => {
      ...
      this.log.debug(
        "Seperated Input Array 	" + JSON.stringify(seperateInputArray)
      );
      ...
      for (const element of seperateInputArray) {
        if (timerObject.zuweisung[element] > 0) {
          ...
          timerToFind = timerObject.zuweisung[element];
          this.log.debug("Timer Number ermittelt: " + timerToFind);
        } else {
      ...
      this.log.debug("Sortet Timerlist: " + JSON.stringify(sortable));
      let pass = 1;
      let timer;
      for (const element of sortable) {
        // Auf Zeit überprüfen
        if (element[1] == timeSec && timerToFind == pass) {
          this.log.debug("pass " + JSON.stringify(pass));
          this.log.debug("TimerToFind " + JSON.stringify(timerToFind));
          this.log.debug("Timer element " + JSON.stringify(element[0]));
          timer = element[0];
        } // Auf Name überprüfen
        else if (element[3] == name && timerToFind == pass) {
          timer = element[0];
        } // Auf Name überprüfen, wenn der Name in der Antwort vor kam
        else if (element[3] == name && timerToFind == 0) {
          timer = element[0];
        } // Auf Device überprüfen
        else if (element[4] == inputDevice && timerToFind == pass) {
          timer = element[0];
    ...
    const startTimer = async (sec, name, inputString) => {
      ...
      timerObject.timer[timerBlock].startTime = startTimer;
    ...
    const generateValues = (timer, sec, index, inputString, name) => {
      ...
      this.log.debug("Onlysec" + JSON.stringify(timer.onlySec));
      ...
      timerObject.interval[index.slice(5)] = setInterval(() => {
        ...
        if (timeLeftSec <= 60 && onlyOneTimer == false) {
          ...
          interval(
            ...
            name,
            timer,
            timerObject.timer[index].timerInterval,
            true
          );
        }
        ...
        if (timeLeftSec <= 0 || timerObject.timerActiv.timer[index] == false) {
          ...
          //TODO - Werte zurücksetzen in Funktion
          timer.hour = "00";
          timer.minute = "00";
          timer.second = "00";
          ...
          timer.lengthTimer = "";
          timer.percent = 0;
          this.log.info(JSON.stringify(index) + " stopped");
    ...
    const getInputDevice = (path) => {
      this.getForeignObject(
        `alexa2.${idInstanze.instanz}.History.name`,
        async (err, obj) => {
          if (err || obj == null) {
            // Error
            ...
            try {
              const obj = await this.getForeignStateAsync(
                `alexa2.${idInstanze.instanz}.History.name`
              );
              let serial;
              if (obj && obj.val) {
                path.inputDevice = obj.val;
                serial = await this.getForeignStateAsync(
                  `alexa2.${idInstanze.instanz}.History.serialNumber`
                );
              }
              if (serial && serial.val) {
    ...
    //----------------------------------------------------------------------------------------------------------------------------------------------------
    //ANCHOR Process Input / Input in Teile zerlegen
    /**
     ...
     * @return { (string | number)[]}  String To Calc, Name, Index zum Löschen der Timer(1 = ein Timer, 2 alle Timer), String In Words
     */
    const processInput = (input, log = false) => {
      let stringToCalc = "";
      let stringInWords = "";
      let name = "";
      ...
      input.forEach((element) => {
        ...
        // Nach Elementen suchen die die Menge der zu löschenden Timer bestimmen
          ...
          timerObject.timerActiv.condition.deleteTimer.indexOf(element) >= 0
        ) {
        ...
        // Nach Und suchen
          ...
          if (stringToCalc.charAt(stringToCalc.length - 1) !== "+") {
            stringToCalc += "+"; // Und bildet ein Verbindungsglied welches für die berechung ein "+" ist
            stringInWords += "und ";
          }
        }
        ...
        // Nach Sekunden suchen, um die Klammern zu schliessen( Wichtig für z.B. 120 Minuten), aber nur wenn als letztes nicht schon eine Klammer ist
          ...
          data.second.indexOf(element) >= 0 &&
          stringToCalc.charAt(stringToCalc.length - 1) != ")"
        ) {
          stringToCalc += ")";
          stringInWords += "Sekunden ";
        } else if (timerObject.brueche1[element]) {
          // Wenn als letztes vor dem Bruch nichts war, soll die eins hinzugefügt werden
          if (stringToCalc.charAt(stringToCalc.length - 1) == "") {
            stringToCalc += "(1";
          }
          stringToCalc += "*" + timerObject.brueche1[element];
        } else if (timerObject.brueche2[element] > 0) {
          // Wenn als letztes vor dem Bruch nichts war, soll die eins hinzugefügt werden
          if (stringToCalc.charAt(stringToCalc.length - 1) == "") {
            stringToCalc += "(1";
          }
          stringToCalc += "*" + timerObject.brueche2[element] + ")*3600";
        }
        ...
        // Überprüfen ob es sich um Zahlen handelt
          ...
          if (
            timerObject.ziffern.indexOf(
              stringToCalc.charAt(stringToCalc.length - 1)
            ) == -1
          ) {
            // Wenn als letztes ein "faktor für stunde oder minute und +"  ist darf keine zusätzliche klammer eingefügt werden
            if (
              (stringToCalc.charAt(stringToCalc.length - 1) != "*3600+" ||
                stringToCalc.charAt(stringToCalc.length - 1) != "*60+") &&
              stringToCalc.charAt(stringToCalc.length - 3) != "("
            ) {
              stringToCalc += "(" + timerObject.zahlen[element];
            } else {
              stringToCalc += timerObject.zahlen[element];
            }
            stringInWords += timerObject.zahlen[element] + " ";
          ...
          // Wenn nichts zutrifft kann es sich nur noch um den Namen des Timers handeln
          name += element + " ";
        }
      ...
      this.log.debug("Timer in Zahlen umgewandelt: " + stringToCalc);
      ...
      if (stringToCalc.charAt(stringToCalc.length - 1) == "+") {
        stringToCalc = stringToCalc.slice(0, stringToCalc.length - 1);
      }
      if (input.length) {
        // Sind Stunden vorhanden
        if (stringToCalc.includes("*3600")) {
          if (log) this.log.debug("Contains Hours");
          if (log) this.log.debug("TimerString (A): " + stringToCalc);
          ...
          if (log)
            this.log.debug(
              stringToCalc.slice(stringToCalc.length - 5, stringToCalc.length)
            );
          if (
            !stringToCalc.includes("*60") &&
            stringToCalc.slice(stringToCalc.length - 5, stringToCalc.length) !=
              "*3600" &&
            stringToCalc.charAt(stringToCalc.length - 1) != ")"
          ) {
        ...
        if (stringToCalc.includes("*60")) {
          if (log) this.log.debug("Contains Minutes");
          // Wenn zum schluss nicht die Einheit der Minuten "*60" steht
          if (log)
            this.log.debug(
              stringToCalc.slice(stringToCalc.length - 3, stringToCalc.length)
            );
        }

        if (stringToCalc.charAt(0) == ")") {
          if (log) this.log.debug("Klammer vorhanden");
          stringToCalc = stringToCalc.slice(2, stringToCalc.length);
        }
      }
      // Leerzeichen von beiden Seiten entfernen
      name = name.trim();
      return [stringToCalc, name, deleteVal, stringInWords];
    };
    ...
    const findTimer = async (
      ...
      this.log.debug(
        "Anzahl aktiver Timer: " +
          JSON.stringify(timerObject.timerActiv.timerCount)
      );
      let foundedTimer = [];
      ...
      for (const element in timerObject.timer) {
        ...
        this.log.debug(
          "Name im Object " + JSON.stringify(timerObject.timer[element].name)
        );
      ...
      for (const element in timerObject.timer) {
        ...
        this.log.debug("Timerindex " + JSON.stringify(deleteTimerIndex));
        if (deleteTimerIndex == 1) {
          // Einer, mit genauer Zeit, nur einmal vorhanden
          // Einer, und einer ist auch nur gestellt
          if (!questionAlexa) {
            if (
              timerObject.timerActiv.timerCount == 1 &&
              timerObject.timerActiv.timer[element] == true
            ) {
              foundedTimer.push(element);
              this.log.debug("Ohne Namen ohne Zeit, nur einer ist aktiv!");
            } else if (
              timerObject.timerActiv.timerCount == 1 &&
              ((countMatchingNumber == 1 &&
                timerObject.timer[element]["onlySec"] == sec &&
                sec !== 0) ||
                (!sec && name == ""))
            ) {
              foundedTimer.push(element);
            ...
            // Einer, mit genauem Namen
              ...
              timerObject.timer[element]["name"] == name &&
              name !== "" &&
              countMatchingName == 1
            ) {
              foundedTimer.push(element);
            ...
            this.log.debug(
              JSON.stringify("Vorkommen des Namen: " + countMatchingName)
            );
            ...
            // Einer, mit genauer Zeit, mehrmals auf verschiedenen Geräten
              ...
              countMatchingInputDevice != timerObject.timerActiv.timerCount
            ) {
              const name = "";
              const inputDevice = "";
          ...
          if (!questionAlexa) {
            foundedTimer.push(element);
            this.log.debug("Suche, alle Timer hinzufügen");
          } else {
            // Alle, nur die vom eingabe Gerät
            if (
              countMatchingInputDevice != timerObject.timerActiv.timerCount &&
              value.indexOf("nein") != -1
            ) {
              if (timerObject.timer[element].inputDevice == inputDevice) {
                foundedTimer.push(element);
        ...
        if (timer.hour !== undefined) {
          try {
            ...
            this.setStateChanged(
              element + ".alive",
              timerObject.timerActiv.timer[element],
              true
            );
          ...
          if (name == "Timer") {
            ...
            this.setStateChanged(
              element + ".name",
              firstLetterToUpperCase(name) + " Timer",
              true
            );
          }
        }
